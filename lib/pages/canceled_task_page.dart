              ...
              BlocBuilder<TaskBloc, TaskState>(
                builder: (context, state) {
                  if (state is TaskChangedState) {
                    tasksByState = getTaskByState(state.tasks, TaskStates.canceled);
                    if (tasksByState.isEmpty) {
                      return const Center(
                          child: Text(
                            'No Canceled tasks Available',
                                ...
                                child: Row(
                                  children: [
                                    for (var task in item.value)
                                      if (task.taskState == TaskStates.canceled)
                                        TaskDesignGeneral(
                                          taskThemeColor: canceledColor,
                                          taskColor: canceledColor.withOpacity(0.2),
                                          task: task,
                                          menuBody: PopOverBodyCanceled(task: task),
                                        )
                                      else
                                        SizedBox(),
                                  ],
